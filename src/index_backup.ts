...
export default class BEM {
    ...
    element(name?: string): BEM | string {
        if(!!name) {
            return new BEM(this.blockName, name); // 新对象，与原来的互不相关
        }
    ...
    //#region 查询DOM元素
    private ctx: Element | Document = document;
    setDom(_ctx: Element | Document | undefined): BEM {
        this.ctx = _ctx || this.getDefaultDom() || document;
        return this;
    }
    /**
     * @test 获取默认的挂载但是可能不正确
     ...
     */
    private getDefaultDom() {
        const clsname = [...this.modifier2string(), ...this.clsname2string()].map(item => `.${item}`).join();
        const result = document.querySelectorAll(clsname);
        return result.length === 1 ? result[0] : null;
    }
    // querySelectorByElement(element?: string) {
    //     return this.ctx.querySelector(this.getBE());
    // }
    // querySelectorAllByElement(element?: string) {
    //     return this.ctx.querySelectorAll(this.getBE());
    // }
    querySelectorByModifier(modifier: string) {
        return this.ctx.querySelector(`${this.getBE()}${BEM.modifierSeparator}${modifier}`);
    }
    querySelectorAllByModifier(modifier: string) {
        return this.ctx.querySelectorAll(`${this.getBE()}${BEM.modifierSeparator}${modifier}`);
    }
    querySelectorByClass(clsname: string) {
        return this.ctx.querySelector(clsname);
    }
    querySelectorAllByClass(clsname: string) {
        return this.ctx.querySelectorAll(clsname);
    }
    querySelectorByState(state: string, ctx: Element | Document = document) {
        return this.querySelectorByClass(this.state2class(state));
    }
    querySelectorAllByState(state: string, ctx: Element | Document = document) {
        return this.querySelectorAllByClass(this.state2class(state));
    }
    //#endregion
}
